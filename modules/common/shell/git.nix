    ...
    home-manager.users.${config.user} = {
      programs.git = {
        ...
        aliases = {
          ...
          ir = "reset";
          l =
            "log --topo-order --pretty=format:'%C(yellow)%h %C(cyan)%cn %C(blue)%cr%C(reset) %s'";
          ls =
            "log --topo-order --stat --pretty=format:'%C(bold)%C(yellow)Commit:%C(reset) %C(yellow)%H%C(red)%d%n%C(bold)%C(yellow)Author:%C(reset) %C(cyan)%an <%ae>%n%C(bold)%C(yellow)Date:%C(reset)   %C(blue)%ai (%ar)%C(reset)%n%+B'";
          ld =
            "log --topo-order --stat --patch --full-diff --pretty=format:'%C(bold)%C(yellow)Commit:%C(reset) %C(yellow)%H%C(red)%d%n%C(bold)%C(yellow)Author:%C(reset) %C(cyan)%an <%ae>%n%C(bold)%C(yellow)Date:%C(reset)   %C(blue)%ai (%ar)%C(reset)%n%+B'";
          lg =
            "log --graph --pretty=format:'%Cred%h%Creset -%C(yellow)%d%Creset %s %Cgreen(%cr) %C(bold blue)<%an>%Creset' --abbrev-commit";
          lga =
            "log --topo-order --all --graph --pretty=format:'%C(yellow)%h %C(cyan)%cn%C(reset) %s %C(red)%d%C(reset)%n'";
          ...
          rem = "remote rename";
          grep =
            ''log --pretty="format:%Cgreen%H %Cblue%s" --name-status --grep'';
          safereset = ''
          ...
          archive-branch =
            "! f() { git tag archive/$1 $1 && git branch -D $1;}; f";
          head = "log -n1";
          heads =
          ...
          longlog =
            "log --pretty=format:'%C(yellow)%h %C(cyan)%cn %C(blue)%cr%C(reset) %s' --graph";
          fuckit = "reset --hard";
          prune-branches =
            "!f() { git fetch --all --prune; git-delete-merged-branches --effort=3 --branch master; }; f";
        };
